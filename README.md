...
- **NOR TENDERED:** 첫 작업 부두는 입항 원문에 적힌 NOR를 사용합니다. `NOR TENDER`, `NOR TENDERED`, `N.O.R.` 표기 및 다음 줄의 `& NOR TENDERED`도 같은 입항 시각으로 읽습니다. 이후 부두는 **바로 이전 실제 접안 부두의 모든 화물 중 마지막 HOSE OFF**를 사용합니다. 같은 접안의 LOAD/DISCH·코스터별 시트는 NOR를 공유하며, 코스터 사이를 부두 이동으로 처리하지 않습니다. 원문에 별도 NOR가 있으면 비고에도 보존합니다.
- 리포트 검토에서 HOSE OFF를 수정하거나 화물을 추가·삭제하면 후속 부두의 NOR도 갱신됩니다. 자동 NOR는 그 원본 HOSE OFF에서 수정합니다. 이전 부두가 화물 작업 없는 대기 부두이거나 HOSE OFF가 누락·불완전하면 더 먼 부두를 건너뛰어 대입하지 않고 `REVIEW`로 표시합니다. 원문에 없는 NOR ACCEPTED도 `REVIEW`이며, HOSE ON이나 검사 통과 시각을 자동 대체하지 않습니다.
- 기존 SOF XLSX의 시트 순서에는 화물 없는 부두를 포함한 전체 접안 이력이 없으므로, 업로드 시에는 기록된 NOR를 보존합니다. 새 규칙으로 계산하려면 운항 리포트 원문을 분석합니다.
