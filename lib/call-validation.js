...
function validateCall(data) {
  ...
  for (const cargo of data.cargo) for (const field of ['berth','maxDraft','coaster']) {
    if (cargo[field] !== undefined && (typeof cargo[field] !== 'string' || cargo[field].length > 10000)) return `cargo.${field} 값을 확인해 주세요.`;
  }
