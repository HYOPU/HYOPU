...
function sofMatchesCall(report,call) {
  const normalize=value=>String(value||'').toUpperCase().replace(/[^A-Z0-9]/g,'');
  const vessel=value=>normalize(String(value||'').toUpperCase().replace(/^S\./,'STOLT '));
  return vessel(report.fields?.vessel)===vessel(call.vessel)&&normalize(report.fields?.voyage)===normalize(call.voyage)&&normalize(report.fields?.port)===normalize(call.port);
}
...
function validateCall(data) {
  ...
  for (const field of ['vessel','voyage','port','etaRaw','etdRaw','pic','status','notes','activityNotes','vcrFileName','latestReport','reportType','reportReceived']) {
    if (typeof data[field]!=='string' || data[field].length>(['notes','activityNotes','latestReport'].includes(field)?50000:200)) return `${field} 항목을 확인해 주세요.`;
  }
  if(data.proformaNotes!==undefined && (typeof data.proformaNotes!=='string'||data.proformaNotes.length>5000)) return 'proformaNotes 항목을 확인해 주세요.';
  if (!data.vessel.trim() || !data.voyage.trim() || !data.etaRaw.trim()) return '선박명·항차·ETA는 필수입니다.';
  if (!['ULSAN',"P'TAEK",'YOSU','DAESAN','DONGHAE','INCHEON','KUNSAN'].includes(data.port)) return '항만을 확인해 주세요.';
  ...
  const schemas={activities:['done','time','activity','company','note'],cargo:['operation','number','name','bl','ship','tanks','party','berth','maxDraft','coaster','note'],crew:['kind','name','schedule','status','note'],tasks:['done','text','due']};
  ...
  if (data.etaActive !== undefined && typeof data.etaActive !== 'boolean') return 'ETA 표시 상태가 올바르지 않습니다.';
  ...
  if (data.sof!==null && !sofMatchesCall(data.sof,data)) return '연결된 SOF와 선박·항차·항만이 다릅니다. SOF를 초기화하거나 입항 정보를 확인해 주세요.';
  return null;
}
module.exports={validateCall,validSof,sofMatchesCall};
