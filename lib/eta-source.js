const PORTS = new Set(['ULSAN', "P'TAEK", 'YOSU', 'DAESAN', 'DONGHAE', 'INCHEON', 'KUNSAN']);
...
function parseEtaRows(rows) {
  ...
  for (const values of rows.slice(index + 1)) {
    ...
    const hasTableValue = [row.vessel, row.voyage, row.port, row.etaRaw, row.pic].some(Boolean);
    if (!hasTableValue) {
      blankRows += 1;
      if (result.length && blankRows >= 2) break;
      continue;
    }
    blankRows = 0;
    if (result.length && /^(?:RGDS\b|HYOP WOON SHIPPING\b)/i.test(row.vessel)) break;
...
function parseEtaWorkbook(buffer, XLSX = require('xlsx')) {
  ...
  if (header(sheetName) !== 'ETAUPDATESC') throw new Error('첫 번째 ETA UPDATE(SC포함) 시트를 찾지 못했습니다.');
