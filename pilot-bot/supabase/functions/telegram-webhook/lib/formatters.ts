import { sequenceLabel, APPLICATION_STATUSES } from '../../ulsan-pilot-watcher/lib/hyopuQueue.ts';
...
export function formatReply(command:Command,data:any,now=Date.now()):{text:string;markup:any}{
 ...
 if(['queue','today','tomorrow','three','search'].includes(command.name)){
   ...
   for(const r of rows){const application=APPLICATION_STATUSES[r.application_status as keyof typeof APPLICATION_STATUSES]?.label??'상태 확인 필요';
     const status=r.forecast_status==='PROCESSING'?'🔄 PROCESSING':r.forecast_status==='BAD_WEATHER'?'⚠️ BAD WEATHER':r.forecast_status??'공개 상태 미확인';
     text+=`\n${sequenceLabel(Number(r.sequence_no))} ${r.vessel_name}\n${pilotDateTimeLabel(r.pilot_date,r.pilot_time)}\n${r.from_location} → ${r.to_location}\n강취: ${r.mooring_name?.trim()||'미확인'}\n${status} / 신청: ${application}\n${r.is_overdue?'⚠️ 예정시간 경과 / 미완료\n':''}${r.needs_review?'⚠️ 원본 상태·매칭 확인 필요\n':''}${r.forecast_time&&r.forecast_time!==r.pilot_time?`공개 예보시간: ${pilotTimeLabel(r.forecast_time)}\n`:''}`;
   }
   if(!rows.length)text+='\n해당 미완료 일정이 없습니다.';
   text+=`\n조회 ${data.total??0}건 / 전체 미완료 ${data.active??0}건`+footer;
 }else if(command.name==='weather'){
   text=`⚠️ [도선 중단 선박조회]\n현재 총 ${data.bad_weather_count}척\n${weatherLabel(data.weather)}\n`;
   rows.forEach((r:any,i:number)=>{text+=`\n${sequenceLabel((command.page??0)*10+i+1)} ${r.vessel_name}${r.agent==='협운'?' ★ 협운':''}\n${pilotDateTimeLabel(r.pilot_date,r.pilot_time)}\n${r.from_location} → ${r.to_location}\n`;});text+=footer;
 }else if(command.name==='changes'){
   text='🕐 [최근 변경]\n'+rows.map((r:any)=>`${kst(r.detected_at)} ${r.vessel_name}\n${settingLabels[r.event_type]??(r.event_type==='BAD_WEATHER_TO_PROCESSING'?'BAD WEATHER → PROCESSING':r.event_type)}${r.old_time&&r.old_time!==r.pilot_time?` ${pilotTimeLabel(r.old_time)} → ${pilotTimeLabel(r.pilot_time)}`:''}`).join('\n\n')+footer;
