...
export function createVesselWorkspace({ getCall, getSession, saveCall, onSaved }) {
  ...
  let draft=null, dirty=false, tab='overview', original=null, busy=false, opener=null;
  ...
  function close() {
    ...
    dialog.close();draft=null;dirty=false;if(opener?.isConnected)opener.focus();return true;
  }
  ...
  dialog.addEventListener('change',async event=>{
    if(event.target.id!=='latest-report-file')return;
    const file=event.target.files[0];if(!file)return;
    if(file.size>50000){updateSaveStatus('원문은 50KB 이하로 입력해 주세요.');return;}
    try{draft.latestReport=await file.text();markDirty();renderPanel();}catch{updateSaveStatus('파일을 읽지 못했습니다.');}
  });
  dialog.addEventListener('click',async event=>{
    ...
    if(button.id==='reload-call'){
      ...
      try{const latest=await getCall(draft.id,true);if(!latest){updateSaveStatus('아직 공유 저장된 기록이 없습니다.');return;}draft=structuredClone(latest);original=latest;dirty=false;renderPanel();updateSaveStatus();}catch(error){updateSaveStatus(error.message);}
    }
  ...
  window.addEventListener('message',event=>{
    const frame=$('#sof-frame');if(!draft||!frame||event.origin!==location.origin||event.source!==frame.contentWindow)return;
    if(event.data?.type==='hyopu:sof-ready')frame.contentWindow.postMessage({type:'hyopu:sof-context',callId:draft.id,report:draft.sof,raw:draft.latestReport,fields:{vessel:draft.vessel,voyage:draft.voyage,port:draft.port}},location.origin);
